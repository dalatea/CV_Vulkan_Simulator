...
void main() {
    ...
    rayTracedFlare *= flareIntensity;
    rayTracedFlare *= vis * viewFactor * scale;

    //float skyMaskVal = skyMask(uv);
    //rayTracedFlare *= skyMaskVal;

    float sunExclude = smoothstep(0.0, 0.001, distSun);
    rayTracedFlare *= sunExclude;

...
namespace cvsim {
    ...
    SimApp::SimApp(const StressConfig& stressCfg)
        ...
        globalPool =
            ...
            .addPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, SwapChain::MAX_FRAMES_IN_FLIGHT * 12)
            .addPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, SwapChain::MAX_FRAMES_IN_FLIGHT * 24)
            .addPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, SwapChain::MAX_FRAMES_IN_FLIGHT * 4)
            .addPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, SwapChain::MAX_FRAMES_IN_FLIGHT * 20)
            .addPoolSize(VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, SwapChain::MAX_FRAMES_IN_FLIGHT * 4) 
            .build();
        ...
        volumetricPass = std::make_unique<enginev::VolumetricPass>(device);
   ...
   std::string SimApp::getAssetPath(const std::string& filename) {
        const char* ament_path = std::getenv("AMENT_PREFIX_PATH");
        if (ament_path) {  // ← ДОБАВЬТЕ ЭТУ ПРОВЕРКУ!
            std::stringstream ss(ament_path);
            std::string path_item;
            
            while (std::getline(ss, path_item, ':')) {
                std::string full_path = path_item + "/share/cv_simulator/assets/" + filename;
                if (std::filesystem::exists(full_path)) {
                    std::cout << "[DEBUG] Found asset at: " << full_path << std::endl;
                    return full_path;
                }
            }
        }
        ...
        if (len != -1) {
            ...
            std::string path = (exe_dir / "../share/cv_simulator/assets/" / filename).string();
            if (std::filesystem::exists(path)) {
                std::cout << "[DEBUG] Found asset at: " << path << std::endl;
                return path;
            }
        }
        
        std::string fallback = "../../src/cv_simulator/assets/" + filename;
        std::cout << "[DEBUG] Fallback to: " << fallback << std::endl;
        return fallback;
    }
    ...
    void SimApp::run() {
        ...
        struct PerCameraLens {
            std::vector<LensSurfaceGPU> surfacesCpu;
            std::unique_ptr<Buffer> surfacesBuffer;
            std::vector<std::unique_ptr<Buffer>> paramsBuffers;   
            std::vector<VkDescriptorSet> lensDescriptorSets;
            CameraConfig config;
        };
        
        std::vector<PerCameraLens> perCamLens(cameraConfigs_.size());

        for (size_t ci = 0; ci < cameraConfigs_.size(); ++ci) {
            auto& pcl = perCamLens[ci];
            auto& cfg = cameraConfigs_[ci];
            pcl.config = cfg;
            
            for (auto& ls : cfg.lensSurfaces) {
                LensSurfaceGPU gpu{};
                gpu.radius   = ls.radius;
                gpu.z        = ls.z;
                gpu.ior      = ls.ior;
                gpu.aperture = ls.aperture;
                gpu.isStop   = ls.isStop;
                pcl.surfacesCpu.push_back(gpu);
            }
            if (pcl.surfacesCpu.empty()) {
                pcl.surfacesCpu = {
                    {  0.050f, 0.000f, 1.8f, 0.020f, 0, 0,0,0 },
                    { -0.050f, 0.010f, 1.0f, 0.020f, 0, 0,0,0 },
                    {  0.030f, 0.020f, 1.6f, 0.018f, 0, 0,0,0 },
                    { -0.030f, 0.028f, 1.0f, 0.018f, 0, 0,0,0 },
                    {  0.0f,   0.035f, 1.0f, 0.012f, 1, 0,0,0 },
                    {  0.040f, 0.040f, 1.5f, 0.020f, 0, 0,0,0 },
                    { -0.040f, 0.050f, 1.0f, 0.020f, 0, 0,0,0 },
                    {  0.035f, 0.058f, 1.5f, 0.018f, 0, 0,0,0 },
                    { -0.035f, 0.065f, 1.0f, 0.018f, 0, 0,0,0 },
                };
            }

            pcl.surfacesBuffer = std::make_unique<Buffer>(
                device,
                sizeof(LensSurfaceGPU),
                static_cast<uint32_t>(pcl.surfacesCpu.size()),
                VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
            );
            pcl.surfacesBuffer->map();
            pcl.surfacesBuffer->writeToBuffer(pcl.surfacesCpu.data());
            pcl.surfacesBuffer->flush();

            pcl.paramsBuffers.resize(SwapChain::MAX_FRAMES_IN_FLIGHT);
            for (int i = 0; i < static_cast<int>(pcl.paramsBuffers.size()); ++i) {
                pcl.paramsBuffers[i] = std::make_unique<Buffer>(
                    device,
                    sizeof(LensParamsGPU),
                    1,
                    VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT
                );
                pcl.paramsBuffers[i]->map();
            }
            
            pcl.lensDescriptorSets.resize(SwapChain::MAX_FRAMES_IN_FLIGHT);
        }
        ...
        auto lensSetLayout =
            ...
            .addBinding(4, VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, VK_SHADER_STAGE_COMPUTE_BIT)
            .build();
...
*/
        ...
        std::vector<VkDescriptorSet> volDescriptorSets(SwapChain::MAX_FRAMES_IN_FLIGHT);
...
*/
        ...
        VkExtent2D renderExtent = extent;
        if (cameraConfigs_[0].resolutionW > 0 && cameraConfigs_[0].resolutionH > 0) {
            renderExtent.width  = static_cast<uint32_t>(cameraConfigs_[0].resolutionW);
            renderExtent.height = static_cast<uint32_t>(cameraConfigs_[0].resolutionH);
        }

        scenePass->recreate(renderExtent);
        bloomPass->recreate(renderExtent, 1.0f);
        lensFlarePass->recreate(renderExtent, 1.0f);
        volumetricPass->recreate(renderExtent, 1.0f);

        auto rebuildAllDescriptors = [&]() {
            ...
            VkDescriptorImageInfo godRaysSampledInfo{};
            godRaysSampledInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            godRaysSampledInfo.imageView = volumetricPass->getView();
            godRaysSampledInfo.sampler = volumetricPass->getSampler();
            ...
            for (int i = 0; i < SwapChain::MAX_FRAMES_IN_FLIGHT; ++i) {
                ...
                enginev::DescriptorWriter(*postSetLayout, *globalPool)
                    ...
                    .writeImage(5, &godRaysSampledInfo)
            ...
            for (auto& pcl : perCamLens) {
                for (int i = 0; i < SwapChain::MAX_FRAMES_IN_FLIGHT; ++i) {
                    ...
                    auto lensSurfInfo   = pcl.surfacesBuffer->descriptorInfo();
                    auto lensParamsInfo = pcl.paramsBuffers[i]->descriptorInfo();
                    ...
                    DescriptorWriter(*lensSetLayout, *globalPool)
                        ...
                        .build(pcl.lensDescriptorSets[i]);
                    ...
                    if (tlas != VK_NULL_HANDLE) {
                        ...
                        writeSet.dstSet          = pcl.lensDescriptorSets[i];
            ...
            for (int i = 0; i < SwapChain::MAX_FRAMES_IN_FLIGHT; ++i) {
                VkDescriptorImageInfo godRaysStorageInfo{};
                godRaysStorageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
                godRaysStorageInfo.imageView   = volumetricPass->getView();
                godRaysStorageInfo.sampler     = VK_NULL_HANDLE;
 
                auto globalInfo = uboBuffers[i]->descriptorInfo();
 
                DescriptorWriter(*volSetLayout, *globalPool)
                    .writeImage (0, &godRaysStorageInfo)
                    .writeBuffer(2, &globalInfo)
                    .build(volDescriptorSets[i]);
                ...
                if (tlas != VK_NULL_HANDLE) {
                    ...
                    writeSet.sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
                    writeSet.pNext           = &tlasInfo;
                    writeSet.dstSet          = volDescriptorSets[i];
        ...
        std::cout << "[DEBUG] Starting BLAS build for " << simObjects.size() << " objects" << std::endl;

        int blasBuilt = 0;
        for (auto& kv : simObjects) {
            if (kv.second.model && kv.second.model->blas == VK_NULL_HANDLE) {
                std::cout << "[DEBUG] Building BLAS for object id=" << kv.first << std::endl;
                kv.second.model->buildBlas(device);
                std::cout << "[DEBUG] BLAS built, handle=" << kv.second.model->blas << std::endl;
                ++blasBuilt;
            }
        }
        std::cout << "[DEBUG] BLAS done, built=" << blasBuilt << ". Building TLAS..." << std::endl;

        tlas = buildTlas(tlasBuffer, tlasMemory);

        rebuildAllDescriptors();

        std::cout << "[DEBUG] TLAS done, handle=" << tlas << std::endl;
        ...
        /*for (int i = 0; i < SwapChain::MAX_FRAMES_IN_FLIGHT; ++i) {
            ...
            DescriptorWriter(*lensSetLayout, *globalPool)
             ...
             if (tlas != VK_NULL_HANDLE) {
                ...
                writeSet.sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
                writeSet.pNext           = &tlasInfo;
                writeSet.dstSet          = lensDescriptorSets[i];
                writeSet.dstBinding      = 4;
                writeSet.dstArrayElement = 0;
                writeSet.descriptorCount = 1;
                writeSet.descriptorType  = VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR;
        ...
        for (int i = 0; i < SwapChain::MAX_FRAMES_IN_FLIGHT; ++i) {
            ...
            // binding 0 = storage image, binding 2 = GlobalUbo
            // binding 1 (TLAS) is written below via vkUpdateDescriptorSets
            DescriptorWriter(*volSetLayout, *globalPool)
                .writeImage (0, &godRaysStorageInfo)
                .writeBuffer(2, &globalInfo)
                .build(volDescriptorSets[i]);
        ...
        cameras.reserve(cameraConfigs_.size());

        for (auto& cfg : cameraConfigs_) {
            CameraControlType ct = (cfg.controlType == "ros")
                ? CameraControlType::ROS
                : CameraControlType::Keyboard;
            
            CameraRig rig = CameraRig::MakeCam(cfg.position, ct);
            rig.rig.transform.rotation = cfg.rotation;
        ...
        for (auto& cfg : cameraConfigs_) {
            ros.addCamera(cfg.name);
            std::cout << "[ROS] Camera '" << cfg.name 
                      << "' → /sim/" << cfg.name << "/image"
                      << ", /sim/" << cfg.name << "/cmd" << std::endl;
        }
        ...
        std::cout << "[DEBUG] Before recreateCaptures" << std::endl; std::cout.flush();
        recreateCaptures();
        std::cout << "[DEBUG] After recreateCaptures" << std::endl; std::cout.flush();
        ...
        std::cout << "[DEBUG] Entering while loop" << std::endl; std::cout.flush();
        while (!window.shouldClose()) {
            std::cout << "[DEBUG] Frame start" << std::endl; std::cout.flush();
            glfwPollEvents();
            ...
            if (cPressed && !cWasPressed) {
                ...
                auto& newCfg = cameraConfigs_[activeCam];
                VkExtent2D newRender;
                newRender.width  = (newCfg.resolutionW > 0) ? static_cast<uint32_t>(newCfg.resolutionW) : extent.width;
                newRender.height = (newCfg.resolutionH > 0) ? static_cast<uint32_t>(newCfg.resolutionH) : extent.height;

                if (newRender.width != renderExtent.width || newRender.height != renderExtent.height) {
                    vkDeviceWaitIdle(device.device());
                    renderExtent = newRender;
                    ...
                    std::cout << "[CAM] Switched to '" << newCfg.name
                            << "' render: " << renderExtent.width << "x" << renderExtent.height << std::endl;
                }
            }
            ...
            for (size_t i = 0; i < cameras.size(); ++i)
            {
                ...
                if (static_cast<int>(i) == activeCam)
                {
                    if (cam.control == CameraControlType::Keyboard) {
                        ...
                        auto cmd = ros.getLastCmd(static_cast<int>(i));
                        cam.applyRos(frameTime, cmd);
                    }
                } else if (cam.control == CameraControlType::ROS) {
                    // ══ НОВОЕ: неактивные ROS-камеры тоже обновляются
                    auto cmd = ros.getLastCmd(static_cast<int>(i));
                    cam.applyRos(frameTime, cmd);
                }
            ...
            for (size_t i = 0; i < cameras.size(); ++i) {
                auto& cfg = cameraConfigs_[i];
                cameras[i].camera.setPerspectiveProjection(
                    glm::radians(cfg.fovDeg),
                    aspect,
                    cfg.nearClip,
                    cfg.farClip);
            }
            ...
            auto& activeCfg = cameraConfigs_[activeCam];
            auto& activeLens = perCamLens[activeCam];

            auto commandBuffer = renderer.beginFrame();
            if (commandBuffer) {
                std::cout << "[DEBUG] beginFrame OK" << std::endl; std::cout.flush();
                ...
                newExtent.height != extent.height) {
                    ...
                    auto& cfg = cameraConfigs_[activeCam];
                    if (cfg.resolutionW <= 0 || cfg.resolutionH <= 0) {
                        renderExtent = extent;
                    }
                ...
                sunViewDir = glm::normalize(-lightDir);
                ...
                lensParams.surfaceCount = static_cast<int>(activeLens.surfacesCpu.size());
                lensParams.sensorZ      = activeCfg.sensorZ;
                lensParams.sensorW      = activeCfg.sensorW;
                lensParams.sensorH      = activeCfg.sensorH;

                lensParams.dustDensity   = activeCfg.contamination.dustDensity;
                lensParams.smudgeAmount  = activeCfg.contamination.smudgeAmount;
                lensParams.scratchAmount = activeCfg.contamination.scratchAmount;
                lensParams.waterDroplets = activeCfg.contamination.waterDroplets;
                lensParams.scatterFactor = activeCfg.contamination.scatterFactor;
 
                activeLens.paramsBuffers[frameIndex]->writeToBuffer(&lensParams);
                activeLens.paramsBuffers[frameIndex]->flush();
                ...
                std::cout << "[DEBUG] shadow OK" << std::endl; std::cout.flush();
                ...
                float scatterBoost = activeCfg.contamination.scatterFactor;
                brightPC.threshold = glm::max(0.3f, brightPC.threshold - scatterBoost * 0.4f);
                ...
                blurPC.texelSize = {
                    1.0f / renderExtent.width,
                     1.0f / renderExtent.height
                };
                blurPC.radius = 5.0f + scatterBoost * 8.0f;
                ...
                bloomPass->endBlurV(commandBuffer);
                 std::cout << "[DEBUG] bloom OK" << std::endl; std::cout.flush();

                lensFlarePass->transitionToGeneral(commandBuffer);

                lensFlarePass->dispatch(commandBuffer, activeLens.lensDescriptorSets[frameIndex]);
                std::cout << "[DEBUG] lensFlare dispatch OK" << std::endl; std::cout.flush();

                volumetricPass->transitionToGeneral(commandBuffer);

                VolPushConstants volPC{};
                volPC.texelSize       = { 1.0f / static_cast<float>(renderExtent.width),
                                        1.0f / static_cast<float>(renderExtent.height) };
                volPC.scatteringCoeff = 0.03f;
                volPC.absorptionCoeff = 0.005f;
                volPC.numSteps        = 48;
                volPC.maxDistance     = 100.0f;
                volPC.dustDensity     = activeCfg.contamination.dustDensity;
                volPC.scatterFactor   = activeCfg.contamination.scatterFactor;
                
                volumetricPass->dispatch(commandBuffer, volDescriptorSets[frameIndex],
                                     &volPC, static_cast<uint32_t>(sizeof(volPC)));

                volumetricPass->transitionToShaderRead(commandBuffer);
                ...
                std::cout << "[DEBUG] beginSwapChainRenderPass OK" << std::endl; std::cout.flush();

                PostProcessPushConstant postPC{};
                postPC.dustDensity   = activeCfg.contamination.dustDensity;
                postPC.smudgeAmount  = activeCfg.contamination.smudgeAmount;
                postPC.scratchAmount = activeCfg.contamination.scratchAmount;
                postPC.waterDroplets = activeCfg.contamination.waterDroplets;
                postPC.scatterFactor = activeCfg.contamination.scatterFactor;

                postProcessSystem.render(frameInfo, postDescriptorSets[frameIndex], postPC);
                
                renderer.endSwapChainRenderPass(commandBuffer);
                std::cout << "[DEBUG] endSwapChainRenderPass OK" << std::endl; std::cout.flush();

                renderer.copySwapImageToBuffer(commandBuffer, captures[frameIndex].buf);
                renderer.endFrame();
                 std::cout << "[DEBUG] endFrame OK" << std::endl; std::cout.flush();
    ...
    void SimApp::loadSimObjects() {
        ...
        const std::string path = (!stressCfg_.scenePath.empty()) 
            ? getAssetPath(stressCfg_.scenePath)
            : getAssetPath("scene_config.json");

        std::cout << "[DEBUG] Trying to open scene_config at: " << path << std::endl;
        std::cout << "[DEBUG] File exists: " << (std::filesystem::exists(path) ? "YES" : "NO") << std::endl;

        std::ifstream file(path);
        if (!file.is_open()) {
            throw std::runtime_error("Can't open scene_config.json" + path);
        }
        ...
        if (scene.contains("cameras")) {
            int idx = 0;
            for (auto& camJson : scene["cameras"]) {
                CameraConfig cfg;
                cfg.id   = idx;
                cfg.name = camJson.value("name", "camera_" + std::to_string(idx));
 
                cfg.controlType = camJson.value("control", "keyboard");
 
                if (camJson.contains("position")) {
                    cfg.position = {
                        camJson["position"][0].get<float>(),
                        camJson["position"][1].get<float>(),
                        camJson["position"][2].get<float>()
                    };
                }
                if (camJson.contains("rotation")) {
                    cfg.rotation = {
                        camJson["rotation"][0].get<float>(),
                        camJson["rotation"][1].get<float>(),
                        camJson["rotation"][2].get<float>()
                    };
                }
                if (camJson.contains("resolution")) {
                    cfg.resolutionW = camJson["resolution"][0].get<int>();
                    cfg.resolutionH = camJson["resolution"][1].get<int>();
                }
                cfg.fovDeg   = camJson.value("fov", 50.0f);
                cfg.nearClip = camJson.value("near", 0.1f);
                cfg.farClip  = camJson.value("far", 100.0f);
 
                if (camJson.contains("sensor")) {
                    auto& s   = camJson["sensor"];
                    cfg.sensorZ = s.value("z", 0.075f);
                    cfg.sensorW = s.value("width", 0.036f);
                    cfg.sensorH = s.value("height", 0.024f);
                }
 
                if (camJson.contains("lensSurfaces")) {
                    for (auto& ls : camJson["lensSurfaces"]) {
                        LensSurfaceConfig lsc;
                        lsc.radius   = ls["radius"].get<float>();
                        lsc.z        = ls["z"].get<float>();
                        lsc.ior      = ls["ior"].get<float>();
                        lsc.aperture = ls["aperture"].get<float>();
                        lsc.isStop   = ls.value("isStop", false) ? 1 : 0;
                        cfg.lensSurfaces.push_back(lsc);
                    }
                }
 
                if (camJson.contains("contamination")) {
                    auto& c = camJson["contamination"];
                    cfg.contamination.dustDensity   = c.value("dustDensity",   0.0f);
                    cfg.contamination.smudgeAmount  = c.value("smudgeAmount",  0.0f);
                    cfg.contamination.scratchAmount = c.value("scratchAmount", 0.0f);
                    cfg.contamination.waterDroplets = c.value("waterDroplets", 0.0f);
                    cfg.contamination.scatterFactor = c.value("scatterFactor", 0.0f);
                }
 
                cameraConfigs_.push_back(std::move(cfg));
 
                std::cout << "[CONFIG] Camera '" << cameraConfigs_.back().name
                          << "': control=" << cameraConfigs_.back().controlType
                          << ", lenses=" << cameraConfigs_.back().lensSurfaces.size()
                          << ", dust=" << cameraConfigs_.back().contamination.dustDensity
                          << std::endl;
 
                ++idx;
            }
        }
        if (cameraConfigs_.empty()) {
            CameraConfig def;
            def.name = "camera_0";
            def.controlType = "keyboard";
            cameraConfigs_.push_back(def);
            std::cout << "[CONFIG] No 'cameras' in config, using default camera" << std::endl;
        }
        ...
        if (stressCfg_.enabled) {
            ...
            std::string modelPath = getAssetPath(stressCfg_.modelPath);
            if (modelPath.empty()) {
        ...
        if (scene.contains("objects")) {
            for (auto& obj : scene["objects"]) {
                std::string modelPath = getAssetPath(obj["model"]);
                ...
                if (obj.contains("texture")) {
                    std::string texPath = getAssetPath(obj["texture"]);
                    simObj.texture = std::make_shared<Texture>(device, texPath);
                }
    ...
    VkAccelerationStructureKHR SimApp::buildTlas(VkBuffer& tlasBuffer, VkDeviceMemory& tlasMemory) {
        ...
        instances.reserve(simObjects.size());
         ...
         if (!Device::vkGetAccelerationStructureBuildSizesKHR ||
        !Device::vkCreateAccelerationStructureKHR ||
        !Device::vkCmdBuildAccelerationStructuresKHR) {
        std::cerr << "[ERROR] Ray tracing extension functions not loaded!" << std::endl;

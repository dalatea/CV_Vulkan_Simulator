...
namespace enginev {
    SkyboxRenderSystem::SkyboxRenderSystem(
        Device& device, VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout, std::string cubeModelPath)
        : device{ device } {
        createPipelineLayout(globalSetLayout);
        createPipeline(renderPass);

        skyboxModel = Model::createModelFromFile(device, cubeModelPath);
    }

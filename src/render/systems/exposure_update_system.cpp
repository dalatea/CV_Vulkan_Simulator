...
namespace enginev {
    ...
    ExposureUpdateSystem::ExposureUpdateSystem(Device& device)
        : device(device)
    {
    ...
    ExposureUpdateSystem::~ExposureUpdateSystem() {
        vkDestroyPipeline(device.device(), pipeline, nullptr);
        vkDestroyPipelineLayout(device.device(), pipelineLayout, nullptr);
        vkDestroyDescriptorSetLayout(device.device(), descriptorSetLayout, nullptr);
    }

    void ExposureUpdateSystem::createDescriptorSetLayout() {
        VkDescriptorSetLayoutBinding exposureBinding{};
        exposureBinding.binding = 0;
        ...
        std::array<VkDescriptorSetLayoutBinding, 2> bindings = { exposureBinding, stateBinding };
    ...
    void ExposureUpdateSystem::createPipelineLayout() {
    ...
    void ExposureUpdateSystem::createPipeline() {

        auto code = readFile("../shaders/exposure_update.comp.spv");
        ...
        if (vkCreateComputePipelines(device.device(), VK_NULL_HANDLE, 1, &info, nullptr, &pipeline) != VK_SUCCESS)
            throw std::runtime_error("failed to create exposure update pipeline");

        vkDestroyShaderModule(device.device(), shaderModule, nullptr);
    }

    void ExposureUpdateSystem::dispatch(VkCommandBuffer cmd, VkDescriptorSet exposureSet) {
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
            pipelineLayout, 0, 1, &exposureSet, 0, nullptr);
